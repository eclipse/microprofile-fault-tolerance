...
This release is a major release of Fault Tolerance. The reason of increasing the release version to 2.0 is that this release upgrades its CDI dependency from CDI 1.2 to CDI 2.0, in order to utilise the new features introduced by CDI 2.0. Therefore, this feature no longer works with Java EE7 features but works with Java EE8 features. However, there are no backward compatibility changes introduced in this specification.
