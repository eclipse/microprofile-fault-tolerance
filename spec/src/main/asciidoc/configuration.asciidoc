...
If an annotation is not present, the configured properties are ignored. For an instance, the property `com.acme.ClassA/methodB/Retry/maxRetries` will be ignored if `Retry` annotation is not specified on the `methodB` of `com.acme.ClassA`. Similarly, the proeprty `com.acme.ClassA/Retry/maxRetries` will be ignored if `Retry` annotation is not specified on the class `com.acme.ClassA` as a class-level annotation.
