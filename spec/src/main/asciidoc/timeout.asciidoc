...
When `@Timeout` is used without `@Asynchronous`, on reaching the specified timeout duration, attempt will be made to interrupt the operation thread. The interruption will only work in certain scenarios. The interruption will not work for the following situations:
...
In the above situations, it is impossible to suspend the execution. The execution thread will finish its process. If the execution takes longer than the specified timeout, the `TimeoutException` will be thrown and the execution result will be discard.
