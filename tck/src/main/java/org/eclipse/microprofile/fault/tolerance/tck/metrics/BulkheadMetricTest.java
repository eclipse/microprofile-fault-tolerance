...
public class BulkheadMetricTest extends Arquillian {
    ...
    public void bulkheadMetricHistogramTest() throws InterruptedException, ExecutionException, TimeoutException {
        ...
        assertThat("histogram count", executionTimes.getCount(), is(2L)); // Rejected executions not recorded in
                                                                          // histogram
        assertThat("median", Math.round(getMedian(snap)), approxMillis(1000));
    ...
    private double getMedian(Snapshot snap) {
        Optional<PercentileValue> value = Arrays.stream(snap.percentileValues())
                .filter(p -> p.getPercentile() == 0.5d)
                .findAny();
        if (!value.isPresent()) {
            fail("No median reported");
        }
