...
public class BulkheadSynchRetryTest extends Arquillian {
    ...
    public void testRetryTestExceptionClass() throws InterruptedException {
        ...
        try (AsyncTaskManager taskManager = new AsyncTaskManager()) {
            // Start taskA
            BarrierTask<Void> taskA = taskManager.runBarrierTask(barrier -> retry1DelayClassBean.test(barrier, new TestException()));
            taskA.assertAwaits();
    ...
    public void testRetryTestExceptionMethod() throws InterruptedException {
        ...
        try (AsyncTaskManager taskManager = new AsyncTaskManager()) {
            // Start taskA
            BarrierTask<Void> taskA = taskManager.runBarrierTask(barrier -> retry1DelayMethodBean.test(barrier, new TestException()));
            taskA.assertAwaits();
    ...
    /**
     * Tests that a Bulkhead + Retry configuration does not retry on
     * BulkheadException
     * <p>
     * The test method should be configured with {@code Bulkhead(1)} and a 1 second
     * delay on Retry.
     * 
     * @param testMethod
     *                       the test method
