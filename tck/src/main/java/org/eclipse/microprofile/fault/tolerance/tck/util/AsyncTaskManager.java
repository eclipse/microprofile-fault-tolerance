 ...
 * <p>
 * This class takes care of creating {@link Barrier Barriers} and
 * {@link BarrierTask BarrierTasks} and ensuring that they get cleaned up.
 * <p>
 * This class implements {@link AutoCloseable} so that it can be used in a
 * try-with-resources block to make it easy to ensure everything from your test
 * is cleaned up.
 * <p>
 * Example:
 * <pre><code>
...
public class AsyncTaskManager implements AutoCloseable {
     ...
     * <p>
     * Since the task is assumed to run synchronously and expected to await the
     * barrier, {@link AsyncCaller} is used to call the task asynchronously.
     * <p>
     * The returned {@link BarrierTask} can be used to release the barrier and
     ...
     * <p>
     * The task is assumed to run asynchronously and return a {@link Future} with
     * its result, either by being a method annotated with {@link Asynchronous} or
     * by some other means.
     * <p>
     * The returned {@link BarrierTask} can be used to release the barrier and
     * assert the result of the task.
     ...
     * Run an asynchronous task which awaits a barrier
     * <p>
     * The task is assumed to run asynchronously and return a
     * {@link CompletionStage} with its result, either by being a method annotated
     * with {@link Asynchronous} or by some other means.
     * <p>
     * The returned {@link BarrierTask} can be used to release the barrier and
     * assert the result of the task.
     * 
     ...
     * <p>
     * The AsyncTaskManager will ensure that {@link Barrier#open()} is called when {@link AsyncTaskManager#close()} is called.
     * 
     ...
     * <p>
     * This method is quicker than calling {@link BarrierTask#assertNotAwaiting()}
    ...
    public static class BarrierTask<T> {
         ...
         * {@link AsyncTaskManager#assertAllNotAwaiting(Collection)} instead.
         */
         ...
         * 
         * @param matcher the {@link Matcher} used to assert the returned value
         */
         ...
         * @throws ExecutionException if the task threw an exception instead of returning
         */
         ...
         * 
         * @param time the time to wait for the task to complete
