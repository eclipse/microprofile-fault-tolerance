 ...
 * <p>
 * All tests should not use this enum directly, but should use {@link MetricGetter} to access metric values.
 * However, if we add new metrics to the spec, their definitions should be added here.
 * Having a defined list of all metrics allows us to easily iterate through them for stuff like {@link MetricGetter#baselineMetrics()}.
 * <p>
 ...
 * <p>
 * Each tag in the set is represented by an enum which implements {@link TagValue}. Each enum has one entry for each valid value for that tag.
 * <p>
 * For example, the metric {@code ft.bulkhead.calls.total} has one tag, {@code bulkheadResult} which can have the value {@code accepted} or
 * {@code rejected}. The value for this is {@link #BULKHEAD_CALLS} and calling {@code BULKHEAD_CALLS.getArgumentClasses()} returns
 * {@link BulkheadResult}, which is an enum with two entries, {@link BulkheadResult#ACCEPTED} and {@link BulkheadResult#REJECTED}.
 */
